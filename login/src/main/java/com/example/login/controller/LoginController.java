...
public class LoginController {
    ...
    public ResponseEntity<LoginResponse> changepw(@RequestBody LoginRequest request) {
        ...
        String firebaseUrl = "https://identitytoolkit.googleapis.com/v1/accounts:sendOobCode?key=" + firebaseApiKey;
        ...
        try {
            restTemplate.postForObject(firebaseUrl, payload, Map.class);
            return ResponseEntity.ok(new LoginResponse(true, "비밀번호 재설정 이메일이 전송되었습니다."));
        } catch (Exception e) {
            e.printStackTrace();
            ...
            if (message.contains("EMAIL_NOT_FOUND")) {
                userMessage = "해당 이메일로 가입된 계정이 없습니다.";
            } else {
                userMessage = "비밀번호 재설정 요청 실패: 알 수 없는 오류가 발생했습니다.";
            }

            return ResponseEntity.status(500).body(new LoginResponse(false, userMessage));
        }
    }
